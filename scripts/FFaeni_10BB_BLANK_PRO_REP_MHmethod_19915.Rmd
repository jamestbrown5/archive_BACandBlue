---
title: "PCoA Visuals and Statistics for VOC from F. faeni in 10% blueberry juice using MH analysis method 199-15"
output:
  html_document:
    df_print: paged
---
...
abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/otu3.csv", colClasses = c("factor","factor", rep("numeric", times= 30)))# read in peak areas, assigning factors of diff classifications based on columns
...
sample_data(VOC)$fmicrobe <- factor(sample_data(VOC)$rep)
#sample_data(VOC)$fnectar <- factor(sample_data(VOC)$juice)

fit <- (sample_sums(VOC)~sample_data(VOC)$fday+sample_data(VOC)$fmicrobe)
anova(fit)
# same results

fit <- lm(sample_sums(VOC)~sample_data(VOC)$fday*sample_data(VOC)$fmicrobe)
#sample_data(VOC)$fnectar
