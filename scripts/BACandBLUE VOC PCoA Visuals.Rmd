...
# read in data
meta <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/data/BACandBLUE_SUBSET_FFvsPA_10BB_PROvsREP/meta.csv") 
meta$hour=as.factor(meta$hour)
rownames(meta) <- meta$SampleID_merge 
SAMP <- sample_data(meta)
sample_names(SAMP)<-meta$SampleID

peaks <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/data/BACandBLUE_SUBSET_FFvsPA_10BB_PROvsREP/peak.csv")
TAX <- tax_table(peaks) # gives a warning, don't freak out
taxa_names(TAX)<-peaks$Peak_ID
colnames(TAX)<- colnames(peaks)

abund <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/data/BACandBLUE_SUBSET_FFvsPA_10BB_PROvsREP/otu.csv", colClasses = c("factor","factor", rep("numeric", times= 24)))
...
fit <- lm(sample_sums(VOC)~ sample_data(VOC)$frep+ sample_data(VOC)$fhour+ 
	+ sample_data(VOC)$fmicrobe)
# same results

fit <- lm(sample_sums(VOC)~ sample_data(VOC)$frep * sample_data(VOC)$fhour * sample_data(VOC)$fmicrobe)





fit <- lm(sample_sums(VOC)~ sample_data(VOC)$rep+ sample_data(VOC)$fhour+ 
...
sample_data(VOC)$rep      3 6.1934e+14 2.0645e+14  1.0593   0.39226    
sample_data(VOC)$fhour    2 2.1685e+16 1.0842e+16 55.6344 3.466e-08 ***
sample_data(VOC)$microbe  1 1.2037e+15 1.2037e+15  6.1764   0.02365 *  
Residuals                17 3.3131e+15 1.9489e+14                      
---
...
TukeyHSD(aov(fit), which="sample_data(VOC)$fhour")
