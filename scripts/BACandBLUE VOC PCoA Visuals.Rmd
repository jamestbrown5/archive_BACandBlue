---
title: "BACandBLUE VOC PCoA Visuals"
author: "Brown, J.T."
...
# read in data
meta <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/stats help/meta.csv") 
rownames(meta) <- meta$SampleID_merge 
SAMP <- sample_data(meta)
sample_names(SAMP)<-meta$SampleID

peaks <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/stats help/peak.csv")
TAX <- tax_table(peaks) # gives a warning, don't freak out
taxa_names(TAX)<-peaks$Peak_ID
colnames(TAX)<- colnames(peaks)

abund <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/stats help/otu.csv", colClasses = c("factor", "factor", rep("numeric", times=24)))
# read in peak areas, assigning factors of diff classifications based on columns
rownames(abund) <- abund$Peak_ID #give abund rows names according to peak ID
...
plot_ordination(VOC, ord, color = "day", shape = "microbe.treatment", type = "samples")
plot_ordination(VOC, ord, type = "split", color = "day", shape = "microbe.treatment", label = "Name")

PCs <- data.frame(scores(ord$vectors))
new.dat <- cbind(PCs, sample_data(VOC)$microbe.treatment, sample_data(VOC)$day, sample_data(VOC)$nectar.conc) # make a new matrix
names(new.dat)[40:42] <- c("microbe.treatment","day","nectar.conc")
library(gdata)
new.dat$Microbe <- reorder.factor(new.dat$microbe.treatment, new.order = c("MrAa", "Aa", "Mr"))
new.dat$Nectar <- reorder.factor(new.dat$nectar.conc, new.order = c("normal", "low"))

ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  geom_point(aes(color = Microbe, shape = Nectar), position ="jitter") +
  facet_grid(~day)+
  theme_bw()+
...
PCs <- data.frame(scores(ord$vectors))
new.dat <- cbind(PCs, sample_data(VOC)$microbe.treatment, sample_data(VOC)$day, sample_data(VOC)$nectar.conc) # make a new matrix
names(new.dat)[24:26] <- c("Microbe", "Day", "Nectar")
library(gdata)
new.dat$Microbe <- reorder.factor(new.dat$Microbe, new.order = c("MrAa", "Aa", "Mr"))
new.dat$Nectar <- reorder.factor(new.dat$Nectar, new.order = c("normal", "low"))
###
###
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  geom_point(aes(color = Microbe, shape = Nectar), position ="jitter") +
...
sample_data(VOC)$fday <- factor(sample_data(VOC)$day)
sample_data(VOC)$fmicrobe <- factor(sample_data(VOC)$microbe.treatment)
sample_data(VOC)$fnectar <- factor(sample_data(VOC)$nectar.conc)
...
#sample_data(VOC)$nectar.conc        1 2.4626e+15 2.4626e+15  25.885 2.986e-06 ***
#sample_data(VOC)$fday               2 3.9403e+15 1.9702e+15  20.708 9.028e-08 ***
#sample_data(VOC)$microbe.treatment  2 4.0738e+15 2.0369e+15  21.410 5.840e-08 ***
#Residuals                          69 6.5646e+15 9.5139e+13                                       
###################################################################################

TukeyHSD(aov(fit), which="sample_data(VOC)$microbe.treatment")
################################################################
...
#$`sample_data(VOC)$microbe.treatment`
#            diff      lwr      upr     p adj
#Mr-Aa   12534704  5425360 19644047 0.0002112
#MrAa-Aa 16353540 10155758 22551322 0.0000001
#MrAa-Mr  3818837 -3146869 10784542 0.3927490
#################################################################

TukeyHSD(aov(fit), which="sample_data(VOC)$nectar.conc") 
##########################################################
#  Tukey multiple comparisons of means
#    95% family-wise confidence level
#
#Fit: aov(formula = fit)
#
#$`sample_data(VOC)$nectar.conc`
#               diff     lwr      upr p adj
#normal-low 11543802 7017335 16070269 3e-06
#
############################################################
...
#Fit: aov(formula = fit)
#
#$`sample_data(VOC)$fday`
#        diff        lwr      upr     p adj
#1-0 11374762  4766512.9 17983010 0.0002992
#2-0 17493356 10885107.4 24101605 0.0000001
#2-1  6118594  -489654.2 12726843 0.0752366
#############################################################
###
##
#
...
Response: sample_sums(VOC)
                                                                                      Df     Sum Sq    Mean Sq F value    Pr(>F)    
sample_data(VOC)$nectar.conc                                                           1 2.4626e+15 2.4626e+15 138.434 < 2.2e-16 ***
sample_data(VOC)$fday                                                                  2 3.9403e+15 1.9702e+15 110.751 < 2.2e-16 ***
sample_data(VOC)$microbe.treatment                                                     2 4.0738e+15 2.0369e+15 114.502 < 2.2e-16 ***
sample_data(VOC)$nectar.conc:sample_data(VOC)$fday                                     2 1.6148e+15 8.0739e+14  45.386 1.618e-12 ***
sample_data(VOC)$nectar.conc:sample_data(VOC)$microbe.treatment                        2 1.0694e+15 5.3468e+14  30.056 1.223e-09 ***
sample_data(VOC)$fday:sample_data(VOC)$microbe.treatment                               4 2.0716e+15 5.1791e+14  29.114 3.385e-13 ***
sample_data(VOC)$nectar.conc:sample_data(VOC)$fday:sample_data(VOC)$microbe.treatment  4 7.9485e+14 1.9871e+14  11.170 9.271e-07 ***
...
fit <- lm(sample_sums(DayTwo)~ sample_data(DayTwo)$nectar.conc * sample_data(DayTwo)$microbe.treatment)
##########################################################################################################
Analysis of Variance Table

Response: sample_sums(DayTwo)
                                                                      Df     Sum Sq    Mean Sq F value    Pr(>F)    
sample_data(DayTwo)$nectar.conc                                        1 2.6684e+15 2.6684e+15  82.249 2.479e-08 ***
sample_data(DayTwo)$microbe.treatment                                  2 4.5758e+15 2.2879e+15  70.521 1.614e-09 ***
sample_data(DayTwo)$nectar.conc:sample_data(DayTwo)$microbe.treatment  2 1.3896e+15 6.9481e+14  21.416 1.354e-05 ***
...
Mrsonly <- subset_samples(VOC, microbe.treatment!="Aa") 
...
# Is there a difference in peak diversity between treatments? Start with all days
...
plot_richness(VOC, x = "microbe.treatment", measures = c("Observed", "Shannon", "Simpson"))

plot_richness(VOC, x = "nectar.conc", measures = c("Observed", "Shannon", "Simpson"))

fit <- lm(estimate_richness(VOC)$Shannon ~ sample_data(VOC)$microbe.treatment)
anova(fit)
####################################################################################
#Analysis of Variance Table
#
...
#                                   Df Sum Sq Mean Sq F value Pr(>F)  
#sample_data(VOC)$microbe.treatment  2 0.7583 0.37916  2.8838 0.0624 .
#Residuals                          72 9.4665 0.13148                              
####################################################################################


fit <- lm(estimate_richness(VOC)$Observed ~ sample_data(VOC)$microbe.treatment)
#####################################################################################
#Analysis of Variance Table
#
#Response: estimate_richness(VOC)$Observed
#                                   Df  Sum Sq Mean Sq F value    Pr(>F)    
#sample_data(VOC)$microbe.treatment  2  477.02 238.510  9.2143 0.0002735 ***
#Residuals                          72 1863.70  25.885                      
######################################################################################

fit <- lm(estimate_richness(VOC)$Simpson ~ sample_data(VOC)$microbe.treatment)
#####################################################################################
#Analysis of Variance Table
#
#Response: estimate_richness(VOC)$Simpson
#                                   Df  Sum Sq  Mean Sq F value  Pr(>F)  
#sample_data(VOC)$microbe.treatment  2 0.16149 0.080744  3.5494 0.03387 *
...
fit <- lm(estimate_richness(VOC)$Shannon ~ sample_data(VOC)$nectar.conc)
#####################################################################################
#Analysis of Variance Table
#
#Response: estimate_richness(VOC)$Shannon
#                             Df  Sum Sq Mean Sq F value Pr(>F)
#sample_data(VOC)$nectar.conc  1  0.1989 0.19886  1.4479 0.2327
#Residuals                    73 10.0259 0.13734               
#####################################################################################

fit <- lm(estimate_richness(VOC)$Observed ~ sample_data(VOC)$nectar.conc)
#####################################################################################
#Analysis of Variance Table
#
#Response: estimate_richness(VOC)$Observed
#                             Df  Sum Sq Mean Sq F value   Pr(>F)   
#sample_data(VOC)$nectar.conc  1  229.75 229.752  7.9451 0.006202 **
#Residuals                    73 2110.97  28.917                    
#####################################################################################

fit <- lm(estimate_richness(VOC)$Simpson ~ sample_data(VOC)$nectar.conc)
#####################################################################################
#Analysis of Variance Table
#
#Response: estimate_richness(VOC)$Simpson
#                             Df  Sum Sq  Mean Sq F value Pr(>F)
#sample_data(VOC)$nectar.conc  1 0.01421 0.014206  0.5809 0.4484
...
# repeat diversity indices calcs with day 0 excluded

NoZero <- subset_samples(VOC, day!="0") 

plot_richness(NoZero, x = "microbe.treatment", measures = c("Observed", "Shannon", "Simpson"))

plot_richness(NoZero, x = "nectar.conc", measures = c("Observed", "Shannon", "Simpson"))

fit <- lm(estimate_richness(NoZero)$Shannon ~ sample_data(NoZero)$microbe.treatment)
#############################################################################################
#Analysis of Variance Table
#
#Response: estimate_richness(NoZero)$Shannon
#                                      Df Sum Sq  Mean Sq F value  Pr(>F)  
#sample_data(NoZero)$microbe.treatment  2 0.4031 0.201558  2.5438 0.08934 .
#Residuals                             47 3.7241 0.079235                  
##############################################################################################


fit <- lm(estimate_richness(NoZero)$Observed ~ sample_data(NoZero)$microbe.treatment)
###############################################################################################
#Analysis of Variance Table
#
#Response: estimate_richness(NoZero)$Observed
#                                      Df Sum Sq Mean Sq F value    Pr(>F)    
#sample_data(NoZero)$microbe.treatment  2 420.01 210.003  26.676 1.813e-08 ***
#Residuals                             47 369.99   7.872                                        
################################################################################################

fit <- lm(estimate_richness(NoZero)$Simpson ~ sample_data(NoZero)$microbe.treatment)
#################################################################################################
#Analysis of Variance Table
#Response: estimate_richness(NoZero)$Simpson
#                                      Df  Sum Sq  Mean Sq F value   Pr(>F)   
#sample_data(NoZero)$microbe.treatment  2 0.14082 0.070409  6.3029 0.003759 **
...
fit <- lm(estimate_richness(NoZero)$Shannon ~ sample_data(NoZero)$nectar.conc)
#################################################################################################
#Analysis of Variance Table
#Response: estimate_richness(NoZero)$Shannon
#                                Df Sum Sq Mean Sq F value  Pr(>F)  
#sample_data(NoZero)$nectar.conc  1 0.3487 0.34865  4.4291 0.04059 *
#Residuals                       48 3.7785 0.07872                  
#################################################################################################

fit <- lm(estimate_richness(NoZero)$Observed ~ sample_data(NoZero)$nectar.conc)
#################################################################################################
#Analysis of Variance Table
#Response: estimate_richness(NoZero)$Observed
#                                Df Sum Sq Mean Sq F value    Pr(>F)    
#sample_data(NoZero)$nectar.conc  1 165.83 165.831  12.753 0.0008204 ***
#Residuals                       48 624.17  13.004                   
#################################################################################################

fit <- lm(estimate_richness(NoZero)$Simpson ~ sample_data(NoZero)$nectar.conc)
#################################################################################################
#Analysis of Variance Table
#Response: estimate_richness(NoZero)$Simpson
#                                Df  Sum Sq  Mean Sq F value   Pr(>F)   
#sample_data(NoZero)$nectar.conc  1 0.10357 0.103566   8.841 0.004596 **
...
# which compounds differentiate our groups? Exclude day 0.

library("DESeq2")
NoZero <- subset_samples(VOC, day!="0") 
deobj <- phyloseq_to_deseq2(NoZero, ~ nectar.conc + microbe.treatment)
diagdds = DESeq(deobj, test="Wald", fitType="local")


res = results(diagdds, contrast = c("microbe.treatment", "Aa", "Mr"), cooksCutoff = FALSE)

res = results(diagdds, contrast = c("microbe.treatment", "Aa", "MrAa"), cooksCutoff = FALSE)

res = results(diagdds, contrast = c("microbe.treatment", "Mr", "MrAa"), cooksCutoff = FALSE)

res = results(diagdds, contrast = c("nectar.conc", "normal", "low"), cooksCutoff = FALSE)
...
x = sort(x, TRUE)
sigtab$Compound = factor(as.character(sigtab$Name), levels=names(x))
sigtab #(res = results(diagdds, contrast = c("microbe.treatment", "Aa", "Mr"), cooksCutoff = FALSE))
#######################################################################################
           baseMean log2FoldChange     lfcSE       stat       pvalue
...
sample_data(NoZero)$TreatNect <- paste(sample_data(NoZero)$microbe.treatment, 
	sample_data(NoZero)$nectar.conc)
...
ggplot(VOC_melt, aes(x=day, y=Abundance, color=microbe.treatment))+
  geom_point()+
  geom_smooth()+
...
ggplot(otu_melt, aes(x= microbe.treatment, y= Abundance, group = microbe.treatment))+
  geom_point()+
  geom_boxplot()+
  facet_wrap(~Name, scales= "free")
...
ggplot(avgtop_melt, aes(x=microbe.treatment, y=Abundance, fill=Name))+
...
ggplot(avg_rare_melt, aes(x=microbe.treatment, y=Abundance, fill=Name))+
	geom_bar(stat = "identity")+
	theme_bw() 
