...
# read in data
meta <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/stats help and prelim practice/meta.csv") 
meta$hour=as.factor(meta$hour)
rownames(meta) <- meta$SampleID_merge 
SAMP <- sample_data(meta)
sample_names(SAMP)<-meta$SampleID

peaks <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/stats help and prelim practice/peak.csv")
TAX <- tax_table(peaks) # gives a warning, don't freak out
taxa_names(TAX)<-peaks$Peak_ID
colnames(TAX)<- colnames(peaks)

abund <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/stats help and prelim practice/otu.csv", colClasses = c("factor","factor", rep("numeric", times=48)))
