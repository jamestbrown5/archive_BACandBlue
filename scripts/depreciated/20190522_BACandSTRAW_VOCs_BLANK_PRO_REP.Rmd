...
## Packages and libraries nessary to complete data analysis.
#### Please consult the appropriate documentation for a comprehensive review of each package's capabilities. 
...
## Organizing the datatables to be compatiable with the phyloseq. These data are exported from MassHunter Quantitate B.07 and the arrangment of the rows and columns have been formatted in excel. 

### Group 1 Dataset includes all of the samples from the VOC collections
...
abund <- read.csv("/Users/jbrown/Documents/GitHub/BACandBlue/data/BACandSTRAW_SUBSET_SWDvsSBERRY_BLANK_PRO_REP/otu1.csv", colClasses = c("factor","factor", rep("numeric", times= 6))) # read in peak areas, assigning factors of diff classifications based on columns
...
names(new.dat)[6:8] <- c("Microbe", "Hour", "Rep")
library(gdata)
new.dat$Microbe <- reorder.factor(new.dat$Microbe, new.order = c("SWD"))
new.dat$Hour <- reorder.factor(new.dat$Hour, new.order = c("24", "48"))
###
