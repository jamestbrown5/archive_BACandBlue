---
title: "PCoA Visuals and Statistics for VOC from F. faeni in 10% blueberry juice using MH analysis method 201-14"
output:
  html_document:
    df_print: paged
---
...
### Group 1: VOC collections from SPME Blanks, Process Blanks, and Treatments)
```{r Group 1: Blanks, Process, Reps, message=TRUE, warning=FALSE, paged.print=TRUE}
###################################################################
...
peaks <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/peak3.csv")
TAX <- tax_table(peaks) # gives a warning, don't freak out
taxa_names(TAX)<-peaks$Peak_ID
colnames(TAX)<- colnames(peaks)

abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/otu7.csv", colClasses = c("factor","factor", rep("numeric", times= 30)))# read in peak areas, assigning factors of diff classifications based on columns

rownames(abund) <- abund$Peak_ID #give abund rows names according to "Peak_ID" in peak3.csv table
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  #facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (71.8% Var. Expl.)")+ 	## !don't forget to update these moving forward!
  ylab("PCoA2 (11.8% Var. Expl.)")
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (59.2% Var. Expl.)")+ 
  ylab("PCoA2 (11.1% Var. Expl.)")
```
...
abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/otu8.csv", colClasses = c("factor","factor", rep("numeric", times= 21)))# read in peak areas, assigning factors of diff classifications based on columns
...
PCs <- data.frame(scores(ord$vectors))
new.dat <- cbind(PCs, sample_data(VOC)$microbe, sample_data(VOC)$rep, sample_data(VOC)$hour) # make a new matrix
names(new.dat)[13:15] <- c("Microbe","Rep","Hour") #for the "must be the same length as vector" error make sure that the info inside the bracets matches empty columns in the dataframe your creating"
library(gdata)
new.dat$Microbe <- reorder.factor(new.dat$Microbe, new.order = c("FF"))
new.dat$Hour <- reorder.factor(new.dat$Hour, new.order = c("2","24","48"))
summary(new.dat)
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  #facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (63% Var. Expl.)")+ 	## !don't forget to update these moving forward!
  ylab("PCoA2 (18% Var. Expl.)")
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (62.5% Var. Expl.)")+ 
  ylab("PCoA2 (15.7% Var. Expl.)")
```

### Group 3: VOC collections from Treatments)
```{r Group 3: Reps Only, echo=TRUE, message=TRUE, warning=FALSE, paged.print=TRUE}
###################################################################
## Organizing the datatables to be compatiable with the phyloseq.## 
###################################################################
...
abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/otu9.csv", colClasses = c("factor","factor", rep("numeric", times= 12)))# read in peak areas, assigning factors of diff classifications based on columns
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  #facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (82.8% Var. Expl.)")+ 	## !don't forget to update these moving forward!
  ylab("PCoA2 (9.5% Var. Expl.)")
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (73.9% Var. Expl.)")+ 
  ylab("PCoA2 (10.1% Var. Expl.)")
```
...
fit <- lm(sample_sums(VOC)~ sample_data(VOC)$hour*sample_data(VOC)$rep)
summary(fit)
######################################################################################################################################
...
#```{r}
res = results(diagdds, contrast = c("hour", "24", "48"), cooksCutoff = FALSE)
res
plotMA(res, ylim=c(-2,2))
idx <- identify(res$baseMean, res$log2FoldChange)
rownames(res)[idx]
#res = results(diagdds, contrast = c("rep", "C01", "R01"), cooksCutoff = FALSE)
#res
#res = results(diagdds, contrast = c("rep", "C01", "R02"), cooksCutoff = FALSE)
#res
#res = results(diagdds, contrast = c("rep", "C01", "R03"), cooksCutoff = FALSE)
#res

alpha = 0.01
sigtab = res[which(res$padj < alpha), ]
sigtab = cbind(as(sigtab, "data.frame"), as(tax_table(NoZero)[rownames(sigtab), ], "matrix"))
x = tapply(sigtab$log2FoldChange, sigtab$Peak_ID, function(x) max(x))
x = sort(x, TRUE)
sigtab$Compound = factor(as.character(sigtab$Peak_ID), levels=names(x))

ggplot(sigtab, aes(y=Compound, x=log2FoldChange)) + 
  geom_vline(xintercept = 0.0, color = "gray", size = 0.5) +
  geom_point(size=3) + 
  theme(axis.text.x = element_text(angle = -90, hjust = 0, vjust=0.5))+
  ggtitle("Compounds signficantly differentiating 24H and 48H VOC collections")+
  theme_bw()
#```
...
ggplot(VOC_melt, aes(x=rep, y=Abundance, color=hour))+
  geom_point()+
  geom_smooth()
###
##
...
sample_data(NoZero)$TreatNect <- paste(sample_data(NoZero)$Peak_ID, 
	sample_data(NoZero)$rep)
...
ggplot(otu_melt, aes(x= rep, y= Abundance, group = hour))+
  geom_point()+
  geom_boxplot()+
...
# 10 most abundant
VOC_avg_top <- prune_taxa(taxa_sums(VOC_avg)>10000000,VOC_avg)
avgtop_melt <-  psmelt((VOC_avg_top))
ggplot(avgtop_melt, aes(x=rep, y=Abundance, fill=Peak_ID))+
...
plot_ordination(VOC, ord, color = "rep", shape = "hour", type = "rep")
