...
## Includes VOC data from F. faeni and P. agglomerans after filtering out both VOCs from the experimental background using CCR updated method of data-walking and removing VOCs that are common between both of the microbes
```{r}
# read in data
...
abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_mix_FFaeni_and_Pagglomerans_10BB_BLANK_PRO_REP/otu.csv", colClasses = c("factor","factor", rep("numeric", times= 44)))
...
abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_mix_FFaeni_and_Pagglomerans_10BB_BLANK_PRO_REP/otu1.csv", colClasses = c("factor","factor", rep("numeric", times= 44)))
# read in peak areas, assigning factors of diff classifications based on columns
rownames(abund) <- abund$Peak_ID #give abund rows names according to "Peak_ID" in peak.csv table
OTU <- otu_table(abund[,-c(1:2)], taxa_are_rows=T) #create a table with only peak areas, compounds are taxa
...
#sample_data(VOC)$rep      3 1.9511e+13 6.5035e+12  1.9273    0.1635    
#sample_data(VOC)$fday     2 4.7967e+14 2.3983e+14 71.0737 5.540e-09 ***
#sample_data(VOC)$microbe  1 3.0177e+14 3.0177e+14 89.4283 3.489e-08 ***
#Residuals                17 5.7366e+13 3.3744e+12                      
#---
...
#$`sample_data(VOC)$microbe`
#         diff     lwr     upr p adj
#PA-FF 7091902 5509672 8674133     0
#################################################################
...
#$`sample_data(VOC)$rep`
#              diff      lwr       upr     p adj
#R02-R01  1458149.7 -1556590 4472889.1 0.5309031
#R03-R01   158088.5 -2856651 3172827.9 0.9987709
#R04-R01 -1082489.0 -4097228 1932250.4 0.7398761
#R03-R02 -1300061.2 -4314801 1714678.3 0.6196520
#R04-R02 -2540638.7 -5555378  474100.8 0.1158626
#R04-R03 -1240577.5 -4255317 1774161.9 0.6531488
############################################################

TukeyHSD(aov(fit), which="sample_data(VOC)$fday")
##SIgnificance found between 2h and 24h and 2h and 48h. Not so much between 24 and 48
###########################################################
#  Tukey multiple comparisons of means
#    95% family-wise confidence level
...
#           diff       lwr      upr     p adj
#24-2   -8747639 -11103876 -6391403 0.0000001
#48-2  -10078808 -12435044 -7722572 0.0000000
#48-24  -1331169  -3687405  1025068 0.3391345
#############################################################
###
##
#
...
#Terms:
#                sample_data(VOC)$fday sample_data(VOC)$microbe
#Sum of Squares           4.796682e+14             3.017705e+14
#Deg. of Freedom                     2                        1
#                sample_data(VOC)$fday:sample_data(VOC)$microbe    Residuals
#Sum of Squares                                    1.555929e+13 6.131674e+13
#Deg. of Freedom                                              2           18
#
#Residual standard error: 1845667
#Estimated effects may be unbalanced             
###################################################################

TukeyHSD(aov(fit), which="sample_data(VOC)$fday")
## 2h and 24H sample significance found ##
#######################################################################################################################################
#  Tukey multiple comparisons of means
#    95% family-wise confidence level
...
#$`sample_data(VOC)$fday`
#           diff       lwr      upr     p adj
#24-2   -8747639 -11102861 -6392418 0.0000001
#48-2  -10078808 -12434029 -7723587 0.0000000
#48-24  -1331169  -3686390  1024053 0.3411441
#######################################################################################################################################
...
#                                                     Df     Sum Sq    Mean Sq
#sample_data(DayTwo)$fday                              1 3.0608e+14 3.0608e+14
#sample_data(DayTwo)$microbe                           1 2.6040e+14 2.6040e+14
#sample_data(DayTwo)$fday:sample_data(DayTwo)$microbe  1 4.1144e+12 4.1144e+12
#Residuals                                            12 4.5304e+13 3.7753e+12
#                                                     F value    Pr(>F)    
#sample_data(DayTwo)$fday                             81.0754 1.099e-06 ***
#sample_data(DayTwo)$microbe                          68.9752 2.558e-06 ***
#sample_data(DayTwo)$fday:sample_data(DayTwo)$microbe  1.0898    0.3171    
#Residuals                                                                 
#---
#Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
################################################################
...
#$`sample_data(DayTwo)$microbe`
#         diff     lwr      upr   p adj
#PA-FF 8068499 5951766 10185232 2.6e-06
################################################################
```
...
#                         Df  Sum Sq  Mean Sq F value   Pr(>F)   
#sample_data(VOC)$microbe  1 0.27994 0.279943  11.392 0.002728 **
#Residuals                22 0.54061 0.024573                    
#---
#Signif. codes:  0 ‘***’ 0.001 ‘**’ 0.01 ‘*’ 0.05 ‘.’ 0.1 ‘ ’ 1
###############################################
...
##############################################
#Call:
#lm(formula = estimate_richness(VOC)$Observed ~ sample_data(VOC)$microbe)

#Coefficients:
#               (Intercept)  sample_data(VOC)$microbePA  
#                     22.92                       -0.75 
###############################################
...
#########################################################
#Call:
#lm(formula = estimate_richness(VOC)$Simpson ~ sample_data(VOC)$microbe)
