...
names(new.dat)[5:7] <- c("Microbe","Juice","Hour") #for the "must be the same length as vector" error make sure that the info inside the bracets matches empty columns in the dataframe your creating"
library(gdata)
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (78.6% Var. Expl.)")+ 	## !don't forget to update these moving forward!
...
ggplot(new.dat, aes(x=Axis.1, y= Axis.2)) +
  ...
  facet_grid(~Hour)+
  theme_bw()+
  xlab("PCoA1 (68.2% Var. Expl.)")+ 
  ylab("PCoA2 (15.3% Var. Expl.)")
```
