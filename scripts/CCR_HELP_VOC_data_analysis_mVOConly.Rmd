...
# read in data
meta <- read.csv("~/GitHub/BACandBlue/STATS HELP CCR/RM_CINM_metadata.csv") 
rownames(meta) <- meta$SampleID_merge 
SAMP <- sample_data(meta)
sample_names(SAMP)<-meta$SampleID

peaks <- read.csv("~/GitHub/BACandBlue/STATS HELP CCR/CINM_peaks.csv")
TAX <- tax_table(peaks) # gives a warning, don't freak out
taxa_names(TAX)<-peaks$Peak_ID
colnames(TAX)<- colnames(peaks)

abund <- read.csv("~/GitHub/BACandBlue/STATS HELP CCR/RM_CINM_otu_t.csv", colClasses = c("factor", "factor", rep("numeric", times=87)))
...
names(new.dat)[40:42] <- c("Microbe", "Day", "Nectar")
library(gdata)
