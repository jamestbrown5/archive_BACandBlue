---
title: "PCoA Visuals and Statistics for VOC from F. faeni in 10% blueberry juice using MH analysis method 200-16"
output:
  html_document:
    df_print: paged
---
...
peaks <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/peak.csv")
TAX <- tax_table(peaks) # gives a warning, don't freak out
taxa_names(TAX)<-peaks$Peak_ID
colnames(TAX)<- colnames(peaks)

abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/otu4.csv", colClasses = c("factor","factor", rep("numeric", times= 30)))# read in peak areas, assigning factors of diff classifications based on columns
...
names(new.dat)[20:22] <- c("Microbe","Rep","Hour") #for the "must be the same length as vector" error make sure that the info inside the bracets matches empty columns in the dataframe your creating"
library(gdata)
...
names(new.dat)[23:25] <- c("Microbe", "Hour", "Rep")
library(gdata)
...
abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/otu5.csv", colClasses = c("factor","factor", rep("numeric", times= 30)))# read in peak areas, assigning factors of diff classifications based on columns
...
abund <- read.csv("~/GitHub/BACandBlue/data/BACandBLUE_GCdata_FFaeni_10BB_BLANK_PRO_REP/otu6.csv", colClasses = c("factor","factor", rep("numeric", times= 30)))# read in peak areas, assigning factors of diff classifications based on columns
